...
class JdockerKafkaResourceIT {
    ...
    void setup() {
        ...
        JdockerKafkaResource kafkaResource = new JdockerKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/jdocker-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/jdocker-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
